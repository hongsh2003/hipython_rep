...
duck_con = duckdb.connect("C:/data_duck/duck_smb.db")

print('b')

duck_con.execute("CREATE TABLE IF NOT EXISTS tb_smb_file AS SELECT * FROM read_csv('c:/data_duck/smb_data/sejong.csv');")
duck_con.execute("DELETE FROM tb_smb_file;")


csv_path = 'c:/data_duck/smb_data'
